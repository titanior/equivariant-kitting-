...
class DQN3DASR(Base3D):
    ...
    def forwardQ2(self, states, in_hand, obs, obs_encoding, pixels, target_net=False, to_cpu=False):
        ...
        plt.imshow(patch[0,0,:,:].detach().cpu().numpy())
        # plt.show()
        q2 = self.q2
    ...
    def decodeActions(self, pixels, a2_id):
        ...
        x = ((pixels[:, 1] - action_pixel_range / 2).float() * self.heightmap_resolution +
             (self.workspace[0][0] + self.workspace[0][1]) / 2).reshape(pixels.size(0), 1)
        y = ((pixels[:, 0] - action_pixel_range / 2).float() * self.heightmap_resolution +
    ...
    def calcTDLoss(self):
        ...
        self.writer.add_scalar('Q1_TD_Loss', q1_td_loss, self.train_step)
        self.writer.add_scalar('Q2_TD_Loss', q2_td_loss, self.train_step)
        self.writer.add_scalar('TD_Loss', td_loss, self.train_step)
        with torch.no_grad():
