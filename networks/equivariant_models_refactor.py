...
class EquShiftQ2ResN(torch.nn.Module):
        ...
        if last_activation_softmax and not args.q2_predict_width:
            self.conv_2 = torch.nn.Sequential(
                nn.R2Conv(nn.FieldType(self.r2_act, n3 * [self.repr]),
                        ...
                        #   kernel_size=8, padding=0, initialize=initialize),
            )
            self.softmax = torch.nn.Softmax(dim=1)
    ...
    def forward(self, obs_encoding, patch):
        ...
        # print("patch_conv_out:",patch_conv_out.shape)
        if args.q2_predict_width:
            ...
            # print("last_activation_softmax True***********************************")
            x = self.conv_2(patch_conv_out).tensor
            # print("1.1 x.shape",x.shape)
            ...
            # print("x.shape: ",x.shape," batch_size: ",batch_size," n_primitives: ",self.n_primitives)
            return x
        else:
            x = self.conv_2(patch_conv_out).tensor
            return x
