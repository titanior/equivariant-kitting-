...
class BlockInsertion(Task):
  ...
  def add_block(self, env):
    ...
    pose = self.get_random_pose_new(env, size , side ='left')
  ...
  def add_fixture(self, env):
    ...
    pose = self.get_random_pose_new(env, size ,side ='right')
    env.add_object(urdf, pose, 'fixed')
    #print('pose',pose)
    return pose
  
  def reset_for_collection(self,env,right = True):
    super().reset(env)
    size = (0.1, 0.1, 0.04)
    urdf = 'insertion/ell.urdf'
    if right:
      ...
      pose = self.get_random_pose_new(env, size , side ='left')
    print("block pose:",pose)
    block_id = env.add_object(urdf, pose)

    urdf = 'insertion/fixture.urdf'
    env.add_object(urdf, pose, 'fixed')
    print("fixture pose:",pose)
    targ_pose = pose
    # self.goals.append(
    #     ([block_id], [2 * np.pi], [[0]], [targ_pose], 'pose', None, 1.))
    self.goals.append(([(block_id, (2 * np.pi, None))], np.int32([[1]]),
