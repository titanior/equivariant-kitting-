...
class Environment(gym.Env):
  ...
  def reset(self,collection = False):
    ...
    # Reset task.
    if collection:
      self.task.reset_for_collection(self)
    else:
      # self.task.reset(self)
      if random.uniform(0,1)<0.5:
        self.task.reset(self)
      else:
        self.task.reset_for_collection(self,right = False)
  ...
  def step_new(self, action=None):
    ...
    # Get task rewards.
    reward, info = self.task.reward_new() if action is not None else (0, {})
    done = self.task.done()
