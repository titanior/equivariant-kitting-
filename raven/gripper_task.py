...
class Task():
  ...
  def reward_new(self):
    ...
    if self.goals:
      ...
      # exceeding 1.0.
      reward = 0.0
    if self.primitive.grasp_success:
      reward = 1
      if self.primitive.detect_coliision:
        reward = 1
    else:
      reward = 0
    return reward, info
  ...
  def get_random_pose_new(self, env, obj_size,side):
    """Get random collision-free object pose within workspace bounds."""

    # Get erosion size of object in pixels.
    max_size = np.sqrt(obj_size[0]**2 + obj_size[1]**2)/2
    # add 5 pixesl to erode size for gripper
    erode_size = int(np.round(max_size / self.pix_size))+20
    ...
    if side == 'left':
      ...
      pos1 = pos[1]
      space_len = 0.02
      if pos0 > 0.5 + space_len:
        pos0 = 0.5 + space_len
      elif pos0 < 0.5 - space_len:
        pos0 = 0.5 - space_len
      pos1 = -np.fabs(pos1)
      if pos1 > -0.25 + space_len:
        pos1 = -0.25 + space_len
      elif pos1 < -0.25 - space_len:
        pos1 = -0.25 - space_len
      pos = (pos0, pos1, obj_size[2] / 2)
      # pos = (0.5, -0.25, obj_size[2] / 2)
    elif side == 'right':
      print("right!!!!!!!!!!")
      pos0 = pos[0]
      pos1 = pos[1]
