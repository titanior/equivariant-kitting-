...
env_group.add_argument('--env', type=str, default='aaoo',
...
if env == 'DualBinFrontRear':
    ...
    print("env is random_household_picking_clutter_full_obs_30------------------------------------------------------------")
    ...
    workspace_size = 0.5
    heightmap_size = 160
    action_space_size = 0.4
    action_pixel_range = heightmap_size * action_space_size / workspace_size
    workspace = np.asarray([[0.5 - workspace_size / 2, 0.5 + workspace_size / 2],
                            [-0.25 - workspace_size / 2, -0.25 + workspace_size / 2],
                            [0, 0 + workspace_size]])
    safe_z_region = 1 / 4
